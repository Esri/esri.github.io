...
function showQueryResults(tags, response) {
    ...
    if (Array.isArray(items) && items.length > 0) {
        ...
        for (i = 0; i < items.length; i ++) {
            ...
            html += '<div class="card card-bar-blue block trailer-1"><div class="card-content"><h4 class="trailer-0">' + project.name + '</h4><div class="font-size--2"><span class="text-green"><b>' + (project.language || '') + '</b></span><span class="font-size--3 text-light-gray right">' + dateUpdated + '</span></div><p class="leader-1">' + (project.description || '') + '</p><div class="leader-1"><span class="icon-fork"></span> ' + project.forks_count + ' <span class="icon-star padding-left-1"> ' + project.stargazers_count + ' </span></div><a href="' + project.html_url + '" class="btn btn-clear text-center leader-1">View it on github</a></div></div>';
        }
